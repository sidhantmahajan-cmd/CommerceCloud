...
public class MediaController extends BaseController{
	...
	public MediaWsDTO uploadMediaImage(
			...
			@RequestParam(required = true, value = "file_field_1") final MultipartFile multiPart,
			@ApiParam(value = "The file representing the actual binary contents of the media to be created.", required = false)
			@RequestBody(required = false) final MultipartFile file_field_2,
		...
		try{
			...
			if (multiPart.getInputStream().available() != 0 && !multiPart.getOriginalFilename().equalsIgnoreCase("file_field_1") && multiPart.getSize()>9){
				...
				if (multiPart.getContentType().equals("application/pdf")){
					throw new WebServiceIOException("Please select image file..");
				...
				throw new WebServiceIOException("Image file is missing..");
			}
